...
$aux3=0;
$aux4=0;
$sql="SELECT * FROM users WHERE grupo=0 and id>100";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        while($row2 = $result2->fetch_assoc()) {
            ...
            while($row3 = $result3->fetch_assoc()) {
                if($row3["operacion"]=="M")$sheet->setCellValue('J'.$i.'', $row3["a"]."x".$row3["b"]);
                if($row3["operacion"]=="D")$sheet->setCellValue('J'.$i.'', $row3["a"]."/".$row3["b"]);
                if($row3["operacion"]=="S")$sheet->setCellValue('J'.$i.'', $row3["a"]."+".$row3["b"]);
                if($row3["operacion"]=="R")$sheet->setCellValue('J'.$i.'', $row3["a"]."-".$row3["b"]);
...
$aux3=0;
$aux4=0;
$sql="SELECT * FROM users WHERE grupo=1 and id>100";
$result = $conexion->query($sql);
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $sheet->setCellValue('A'.$i.'', $row["nombre"]." ".$row["apellido"]);
